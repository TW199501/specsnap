...
- `v0.0.2` — 元素間距 `Gap`
- `v0.0.3` **（current）** — `toAnnotatedPNG` 每 frame PNG、`toSpecSnapBundle` 磁碟 bundle、MD 相對路徑圖片引用

Roadmap：
- `v0.0.4` — consumer-agnostic 檔案系統寫入輔助（FSA API wrapper、extension `chrome.downloads` wrapper），subpixel 顯示潤飾
